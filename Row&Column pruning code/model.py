...
class ZFNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential( 
            nn.Conv2d(3, 48, kernel_size=7, stride=2, padding=1),   # input[3, 224, 224]  output[48, 110, 110]
            nn.BatchNorm2d(48),
            nn.ReLU(inplace=True), 
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),       # output[48, 55, 55] 
...
def get_newRsenet18(num_classes = 10):
    ...
    return model
def get_ZFNet(num_class=10):
    model = ZFNet(num_class)
    return model
