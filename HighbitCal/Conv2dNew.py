...
shift_bit_Dic = {
    'LeNet': [[[5 for i in range(3)], [0 for i in range(3)], [0 for i in range(3)]]],
    'VGG8': [
        # [[0 for i in range(7)], [0 for i in range(7)], [0 for i in range(7)]],
        # [[7 for i in range(7)],[0 for i in range(7)],[0 for i in range(7)]],
        [[2,3,2,3,2,4,4], [0 for i in range(7)], [0 for i in range(7)]],
        [[1 for i in range(7)], [1 for i in range(7)], [1 for i in range(7)]],
        [[2 for i in range(7)], [2 for i in range(7)], [2 for i in range(7)]],
        [[3 for i in range(7)], [3 for i in range(7)], [3 for i in range(7)]],
        [[4 for i in range(7)], [4 for i in range(7)], [4 for i in range(7)]],
        [[5 for i in range(7)], [5 for i in range(7)], [5 for i in range(7)]],
        [[6 for i in range(7)], [6 for i in range(7)], [6 for i in range(7)]],
        [[7 for i in range(7)], [7 for i in range(7)], [7 for i in range(7)]],
    ...
    'VGG16': [
        ...
        [[3 for i in range(13)], [3 for i in range(13)], [3 for i in range(13)]],
        [[4 for i in range(13)], [4 for i in range(13)], [4 for i in range(13)]],
        [[5 for i in range(13)], [5 for i in range(13)], [5 for i in range(13)]],
        [[2 for i in range(13)], [2 for i in range(13)], [2 for i in range(13)]]
    ],
    'AlexNet': [
    ...
    'ResNet': [
        ...
        [[0 for i in range(20)], [0 for i in range(20)], [0 for i in range(20)]],
        [[3 for i in range(20)], [1 for i in range(20)], [0 for i in range(20)]],
        [[4 for i in range(20)], [1 for i in range(20)], [0 for i in range(20)]],
        [[3 for i in range(20)], [2 for i in range(20)], [0 for i in range(20)]],
        [[2 for i in range(20)], [3 for i in range(20)], [0 for i in range(20)]],
        [[1 for i in range(20)], [4 for i in range(20)], [0 for i in range(20)]],
        [[3, 3, 3, 3, 3, 2, 1, 2, 2, 2, 2, 1, 2, 2, 2, 2, 1, 2, 2, 2], [2 for i in range(20)], [0 for i in range(20)]]
                ...
                if (self.compute_mode == 0 or self.compute_mode == 2):
                    if self.layer_count == 0:  # 第1个卷积层直接进行剪枝就好，不用到下一层的预测结果
                        ...
                        if(pruneTensor[0] == 1):
                            ...
                            # print("1 after row pruning:",oneCal(self.input))
                            # 4444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444
    ...
    def activationSlidePrune(self, input, ratio,r_or_c, pattern='Train'):
        ...
        if r_or_c == 1:
            ...
            if pattern == 'test':
                ...
                # print("layer 0 1 after row pruning:",oneCal(self.input))
                #222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
                return input
            ...
            # print("列剪枝比例为：",aaaa1/len(pruneTensor_column))
            ...
            if pattern == 'test':
                ...
                # 222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
                print("layer 0 1 after pruning:", oneCal(self.input))
                # 222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
...
def comNew(input, weight, bias, bits, layer_count, batch_size, a, ratio, model_name, shift_bit_input, shift_bit_weight,
    ...
    if model_name != 'ResNet' and model_name != 'NewResNet':
        ...
        # print("1 in 8 bit:", oneCal(pre_input_8))
        # print("1 in 4 bit:", oneCal(input_4))
        # 6666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
        ...
        # Judge parameter change after ReLU()
        # Relu = nn.ReLU()
        # oo4 = Relu(o_4)
        ...
        if (compute_mode == 0 or compute_mode == 2):
            if layer_count == Model_Dict[model_name]['thre'] - 1:  #last layer
                ...
                temp_out = temp_out.permute(3, 0, 1, 2)  # 矩阵转换为输入特征图的形式
                relu = nn.ReLU()
                temp_out = relu(temp_out)

                pp4 = torch.ones_like(temp_out)
                qq4 = torch.logical_and(pp4, temp_out)  # 进行与操作
                bb4 = (torch.sum(qq4)) / (temp_out.shape[0] * temp_out.shape[1] * temp_out.shape[2] * temp_out.shape[3])

                print('ReLU:', model_name, "layer", layer_count + 1, "low bit ratio = ", bb4)
                if model_name == 'AlexNet' or model_name == 'ZFNet':
                    maxpool = nn.MaxPool2d(3, 2)
                else:
                    maxpool = nn.MaxPool2d(2,2)
                pre_pool = temp_out.shape[0] * temp_out.shape[1] * temp_out.shape[2] * temp_out.shape[3]
                temp_out = maxpool(temp_out)
                after_pool = temp_out.shape[0] * temp_out.shape[1] * temp_out.shape[2] * temp_out.shape[3]
                print('MAXPOOL:',model_name,layer_count+1,"low bit ratio = ",after_pool/pre_pool)
            ...
            if layer_count <= Model_Dict[model_name]['thre'] - 2:  # 最后一个卷积层后面没有pooling操作了
                ...
                if Model_Dict[model_name][str(layer_count + 1)][0] != 0:
                    ...
                    temp_out = maxpool(temp_out)
                    after_pool = temp_out.shape[0] * temp_out.shape[1] * temp_out.shape[2] * temp_out.shape[3]
                    print('MAXPOOL:',model_name,"layer", layer_count + 1,"low bit ratio = ", after_pool / pre_pool)
                    # print("池化后参数总个数为：", temp_out.shape[0] * temp_out.shape[1] * temp_out.shape[2] * temp_out.shape[3])
                # 根据高4bit的计算结果进行下一层的剪枝，先imgcol展开
                # 先对o2进行量化
        ...
        weight_4 = torch.from_numpy(weight_4_np).cuda()
        # 6666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
        # print("1 after column pruning:", oneCal(pre_input_8))
        # print("1 in 4 bit:", oneCal(input_4))
        # 6666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666666
        if bias is not None:
            bias_4 = torch.from_numpy(bias_4_np).cuda()
